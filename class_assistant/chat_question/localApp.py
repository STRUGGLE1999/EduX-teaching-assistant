...
class MyVectorDBConnector:
    ...
    def search(self, query, top_n=5, similarity_threshold=None, mode="auto"):
        ...
        mode 参数:
            - "auto": 默认模式，自动设定阈值并过滤
            - "threshold": 使用指定阈值过滤
            - "topn": 直接取距离最小的前 top_n 条
        """
        # 获取查询的嵌入向量
        query_embedding = self.embedding_fn([query])[0]
        results = self.collection.query(
            query_embeddings=[query_embedding],
            n_results=top_n * 2  # 多拿一些回来，便于后处理
        )
        ...
        print(f"🔎 raw docs: {results['documents']}")
        print(f"🔎 raw distances: {results['distances']}")
        print(f"📈 距离统计 - 最小值: {min_distance:.2f}, 最大值: {max_distance:.2f}, 平均值: {avg_distance:.2f}")
        ...
        if mode == "auto":
            ...
            if similarity_threshold is None:
                similarity_threshold = min_distance + 50  # 容差设定
                print(f"⚙️ 自动设定 similarity_threshold = {similarity_threshold:.2f}")
            ...
            # 手动传入阈值
            if similarity_threshold is None:
                raise ValueError("在 mode='threshold' 模式下，请手动传入 similarity_threshold")
            for doc, score in zip(all_docs, all_distances):
                if score <= similarity_threshold:
                    filtered_results.append((doc, score))
            ...
            combined.sort(key=lambda x: x[1])  # 按距离升序
...
class Chat_Bot:
    def __init__(self, n_results=5, similarity_threshold=3):
        ...
        self.vector_db = None  # 提前设置属性为 None，避免未定义错误
    ...
    def chat(self, user_query):
        if self.vector_db is None:
            raise ValueError("请先调用 createVectorDB(file) 方法加载教材并构建向量数据库。")
        # 1. 检索相关文档，并应用相似度阈值过滤
